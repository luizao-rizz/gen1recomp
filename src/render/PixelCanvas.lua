...
-- Protected against graphics context loss (Android suspend/resume).
function PixelCanvas.new(w, h, filter)
  local ok, canvas = pcall(love.graphics.newCanvas, w, h, { dpiscale = 1 })
  if ok and canvas and canvas.setFilter then
    if filter then canvas:setFilter(filter, filter) end
    return canvas
  end
  return nil  -- Canvas creation failed; return nil instead of crashing
end
