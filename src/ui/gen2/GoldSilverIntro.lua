...
function GoldSilverIntro:bgCanvas()
  ...
  if self.mapDirty then
    ...
    -- Save canvas state before any operations; use pcall to protect against
    -- graphics context loss (common on Android when app suspends/resumes).
    local previous = G.getCanvas()
    G.push()
    G.origin()
    -- Wrap canvas operations in pcall to ensure restoration even if draw fails
    ...
    -- Always restore canvas and state, even if drawing failed
    G.setCanvas(previous)
    G.pop()
    if not ok then
      Logger.error("GoldSilverIntro: canvas render failed", err)
      self.mapDirty = true  -- retry next frame
    else
      self.mapDirty = false
    end
  end
