...
function BattleAnimView:bake(drawBg, palByte)
  ...
  if not self.canvas then
    ...
    if not ok then return end  -- canvas creation failed, skip bake
    canvas:setFilter("nearest", "nearest")
    self.canvas = canvas
  end
  local previousCanvas = G.getCanvas()
  local previousBgp = GbcPalette.setBgp(palByte)
  -- Wrap ALL canvas operations in pcall to guarantee state restoration
  ...
  -- Always restore state, even if canvas operations failed
  G.setCanvas(previousCanvas)
  GbcPalette.setBgp(previousBgp)
  if not ok then
    -- Log error but don't crash; next frame will retry
    local Logger = require("src.core.Logger")
    Logger.error("BattleAnimView: bake failed", err)
  end
end
