...
function YellowIntro:rebuildBgCanvas()
  ...
  -- Wrap canvas operations in pcall to prevent graphics context loss from crashing
  ...
  love.graphics.pop()
  if not ok then
    -- Graphics context issue; retry next frame instead of crashing
    print("YellowIntro canvas rebuild failed (graphics context?), will retry")
  else
    self.bgDirty = false
  end
end
