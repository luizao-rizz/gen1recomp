...
function love.lowmemory()
  ...
  if Game then
    -- Protect against crashes during low memory recovery; memory pressure can
    -- cause operations like garbage collection or resource reallocation to fail
    local ok, err = pcall(function() Game:onResume() end)
    if not ok then
      local Logger = require("src.core.Logger")
      Logger.error("lowmemory: onResume failed (memory critical?)", err)
      -- Try aggressive garbage collection as fallback
